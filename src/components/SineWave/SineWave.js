...
export default class SineWave extends Component {
  ...
  createDocument() {
    ...
    const xScale = scaleLinear().domain([0, 20]).range([0, dimensions.width]);

    const yScale = scaleLinear().domain([0, 20]).range([dimensions.height, 0]);
  ...
  addSineAxis(state) {
    const yAxis = axisLeft(state.yScaleAxis).tickValues([-1, 0, 1]).tickFormat(format('d'));

    state.graphContainer.append('g').attr('class', 'y axis left').attr('transform', `translate(${state.firstAxisXCoord}, 0)`).call(yAxis);
    ...
    state.graphContainer.append('g').attr('class', 'x axis left').call(xAxis);
  }
  ...
  addGraphContainer(container, xScale, yScale) {
    ...
    const graphContainer = container.append('g').attr('class', 'circle-container').attr('transform', `translate(${initialX}, ${initialY})`);

    graphContainer.append('circle').attr('cx', 0).attr('cy', 0).attr('r', radius).attr('class', 'unit-circle').style('fill', 'none');

    this.addRadianNumberLine(graphContainer);

    const hypotenuse = graphContainer.append('line').attr('class', 'hypotenuse').attr('x1', 0).attr('y1', 0).attr('x2', 0).attr('y2', 0);

    const opposite = graphContainer.append('line').attr('class', 'opposite').attr('x1', 0).attr('y1', 0).attr('x2', 0).attr('y2', 0);

    const adjacent = graphContainer.append('line').attr('class', 'adjacent').attr('x1', 0).attr('y1', 0).attr('x2', 0).attr('y2', 0);

    const dot = graphContainer.append('circle').attr('class', 'dot').attr('cx', radius).attr('cy', 0).attr('r', 5);

    const verticalDot = graphContainer.append('circle').attr('cx', 0).attr('cy', 0).attr('r', 5).attr('class', 'vertical-guide');
    ...
    const axisDot = graphContainer.append('circle').attr('cx', radius).attr('cy', 0).attr('r', 5).attr('class', 'axis-dot');
  ...
  drawGraph(state) {
    ...
    state.axisDot.attr('cx', axisDotX).attr('cy', 0);

    const dx = radius * Math.cos(state.time);
    const dy = radius * -Math.sin(state.time); // counter-clockwise

    state.dot.attr('cx', dx).attr('cy', dy);

    state.hypotenuse.attr('x2', dx).attr('y2', dy);

    state.opposite.attr('x1', dx).attr('y1', dy).attr('x2', dx).attr('y2', 0);

    state.adjacent.attr('x1', dx).attr('y1', 0);

    state.verticalDot.attr('cy', dy);

    state.joiningLine.attr('y1', state.dot.attr('cy')).attr('x2', state.dot.attr('cx')).attr('y2', state.dot.attr('cy'));
  ...
  drawSineWave(state) {
    ...
    state.graphContainer.append('path').datum(sineData).attr('class', 'sine-curve').attr('d', sine);
  }
  ...
  addRadianNumberLine(container) {
    [
      ...
      container.append('line').attr('x1', 0).attr('y1', 0).attr('x2', cosX).attr('y2', sinY);
    });
  }
  ...
  render() {
    return (
      <Grid>
        <Row>
          <Col sm={12}>
            <Col xsOffset={3} mdOffset={1} componentClass="h2">
              sin(x)
            </Col>
            <div
              id="sine-wave"
